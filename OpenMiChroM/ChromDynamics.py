...
try:
    ...
    print(
        "Unable to load OpenMM as 'openmm'. Trying 'simtk.openmm'..."
    )
    try:
        ...
        raise ImportError(
            "Failed to load OpenMM. Please check your installation and configuration."
        )
...
class MiChroM:
    ...
    def setup(
        ...
        if precision not in ["mixed", "single", "double"]:
            raise ValueError(
                "Precision must be 'mixed', 'single', or 'double'."
            )
        ...
        platform_priority = [preferred_platform] + [
            p
            for p in default_platforms.values()
            if p != preferred_platform
        ]

        # Dictionary to map platform names to their specific property names
        property_names = {
            "CUDA": {
                "DeviceIndex": "CudaDeviceIndex",
                "Precision": "CudaPrecision",
            },
            "HIP": {
                "DeviceIndex": "HipDeviceIndex",
                "Precision": "HipPrecision",
            },
            "OpenCL": {
                "DeviceIndex": "OpenCLDeviceIndex",
        ...
        for plat_name in platform_priority:
            try:
                self.platform = self.mm.Platform.getPlatformByName(
                    plat_name
                )
                ...
                if plat_name != "CPU":
                    if self.gpu.lower() != "default":
                        properties[
                            property_names[plat_name]["DeviceIndex"]
                        ] = deviceIndex
                    properties[
                        property_names[plat_name]["Precision"]
                    ] = precision
            ...
            raise Exception(
                "No suitable computational platform is available."
            )
    ...
    def loadStructure(self, data, center=True, massList=None):
        ...
        if massList is None:
            ...
            if len(massList) != self.N:
                raise ValueError(
                    f"Mass list length {len(massList)} does not match number of beads {self.N}."
                )
    ...
    def setPositions(
        self, beadsPos, randomize=False, randomOffset=1e-5
    ):
        ...
        if randomize:
            data += (
                np.random.random(data.shape) * 2 - 1
            ) * randomOffset
    ...
    ##      FORCES
    ##============================
    def addCylindricalConfinement(
        self, rConf=5.0, zConf=10.0, kConf=30.0
    ):
        ...
        for i in range(self.N):
            self.forceDict["CylindricalConfinement"].addParticle(
                i, []
            )
    ...
    def addFENEBonds(self, kFb=30.0, bonds=None, chainIndices=None):
        ...
        if "FENEBond" not in self.forceDict:
            ...
            feneBondForce.addGlobalParameter(
                "cutoff", 2.0 ** (1.0 / 6.0)
            )
        ...
        if bonds is None:
            ...
            for idx in chainIndices:
                ...
                for j in range(start, end):
                    ...
                    self.forceDict["FENEBond"].addBond(
                        int(i1), int(i2), []
                    )
    ...
    def addAngles(self, kA=2.0, chainIndices=None):
        ...
        *The Journal of Chemical Physics*, 134(20), 204904.
        ...
        for idx in chainIndices:
            ...
            if isRing:
                num_angles += (
                    2  # For the two additional angles in a ring
                )
    ...
    def addCustomTypes(
        ...
        energy = (
            "mapType(t1,t2)*0.5*(1. + tanh(mu*(rc - r)))*step(r-lim)"
        )
    ...
    def _createTypeList(self, header_types):
        ...
        self.type_list = [
            typesDict[letter] for letter in self.type_list_letter
        ]
    ...
    def addCustomIC(
        ...
        IClist = np.append(np.zeros(dinit), IClist_listfromfile)[
            :-dinit
        ]
    ...
    def addCustomMultiChainIC(
        ...
        energyIC = (
            "step(d-dinit)*ic(d)*step(dend-d)*f;"
            "f=0.5*(1. + tanh(mu*(rc - r)));"
            "d=abs(idx1-idx2)"
        )
    ...
    def addMultiChainIC(
        ...
        self.forceDict[
            "IdealChromosomeChain{0}".format(chainIndex)
        ] = IC
    ...
    def addCorrelatedNoise(self, act_seq="none", atol=1e-5):
        ...
        try:
            ...
            act_force = self.mm.CustomExternalForce(
                " - F_act * (x + y + z)"
            )
            ...
            for bead_id, Fval in enumerate(act_seq):
                if Fval > atol:
                    self.forceDict["ActiveForce"].addParticle(
                        int(bead_id), [Fval]
                    )
            ...
            ==================================\n".format(
                    self.integrator.corrTime,
                    self.forceDict["ActiveForce"].getNumParticles(),
                    self.N,
                )
            )
            ...
            print(
                "Structure not loaded! Load structure before adding activity.\nCorrelated noise has NOT been added!"
            )

        except (ValueError, AssertionError):
            print(
                "Active sequence (act_seq) either not defined or all the monomers are not accounted for!\nCorrelated noise has NOT been added!"
            )

    def addHarmonicBonds(
        self, bondStiffness=30.0, equilibriumDistance=1.0
    ):
        R"""
        ...
        if "HarmonicBond" not in self.forceDict:
            ...
            harmonicBondForce = self.mm.CustomBondForce(
                bondForceExpression
            )
            harmonicBondForce.addGlobalParameter("kfb", bondStiffness)
            harmonicBondForce.addGlobalParameter(
                "r0", equilibriumDistance
            )
    ...
    def addSelfAvoidance(self, Ecut=4.0, k_rep=20.0, r0=1.0):
        ...
        repul_energy = (
            "0.5 * Ecut * (1.0 + tanh(1.0 - (k_rep * (r - r0))))"
        )

        self.forceDict["SelfAvoidance"] = (
            self.mm.CustomNonbondedForce(repul_energy)
        )
    ...
    def _getForceIndex(self, forceName):
        ...
        index = [
            i
            for i, systemForce in enumerate(self.system.getForces())
            if systemForce.this == forceObject.this
        ]
        ...
        if len(index) == 1:
            ...
            raise Exception(
                "Found more than one force with input name!"
            )
    ...
    def removeForce(self, forceName):
        ...
        if forceName in self.forceDict:
            ...
            raise ValueError(
                "The system does not have force {0}.\nThe forces applied in the system are: {}\n".format(
                    forceName, self.forceDict.keys()
                )
            )
    ...
    def addAdditionalForce(self, forceFunction, *args, **kwargs):
        ...
        if not isinstance(forceFunction, types.MethodType):
            warnings.warn(
                "Using user defined force function. Make sure to include the new force object in the MiChroM.forceDict dictionary."
            )
        ...
        newKeys = list(
            set(oldForceDictKeys) ^ set(self.forceDict.keys())
        )
        ...
        try:
            ...
            if newForceDictKey is None:
                raise ValueError(
                    "No new force in MiChroM.forceDict! The new force is either already present or was not added properly to the force dictionary."
                )
            if newKeys:
                raise ValueError(
                    "Force function added multiple new forces in MiChroM! Please break down the function so each force is added separately."
                )
        ...
        for name in self.forceDict.keys():
            ...
            if force_group > 31:
                force_group = 31
                print(
                    "Attention, force was added to Force Group 31 because no other was available."
                )
    ...
    def run(
        self, nsteps, report=True, interval=10**4, totalSteps=None
    ):
        R"""
    ...
    def addHomoPolymerForces(
        self, harmonic=False, kFb=30, kA=2.0, eCut=4.0
    ):
        R"""
    ...
    def loadNDB(self, NDBfiles=None):
        ...
        if set(typesLetter) == {"SQ"}:
            typesLetter = [
                "bead{:}".format(x)
                for x in range(1, len(typesLetter) + 1)
            ]
    ...
    def loadGRO(self, GROfiles=None, ChromSeq=None):
        ...
        if not ChromSeq is None:
            ...
            if len(ChromSeq) != len(GROfiles):
                raise ValueError(
                    "Number of sequence files provided must agree with number of coordinate files!"
                )
        ...
        if len(typesLetter) != len(x):
            raise ValueError(
                "Sequence length is different from coordinates length!"
            )
    ...
    def createSpringSpiral(
        self, ChromSeq=None, isRing=False, chromosome=None
    ):
        R"""
        ...
        for i in range(beads):
            ...
            b = 1 / np.sqrt(
                (4 - 3.0 * np.cos(a1) - np.cos(10 * a1) * np.cos(a1))
                ** 2
                + (
                    0
                    - 3.0 * np.sin(a1)
                    - np.cos(10 * a1) * np.sin(a1)
                )
                ** 2
                + (np.sin(10 * a1)) ** 2
            )
            ...
            x.append(
                1.5 * np.pi * b
                + 3 * b * np.cos(a)
                + b * np.cos(10 * a) * np.cos(a)
            )
            y.append(
                1.5 * np.pi * b
                + 3.0 * b * np.sin(a)
                + b * np.cos(10 * a) * np.sin(a)
            )
    ...
    def createRandomWalk(self, ChromSeq=None, chromosome=None):
        ...
        theta = np.repeat(
            np.random.uniform(0.0, 1.0, Nbeads // segment_length + 1),
            segment_length,
        )
    ...
    def initStructure(
        ...
        if mode == "auto":
            if CoordFiles is None:
                ...
                if extension == ".pdb":
                    ...
                    raise ValueError(
                        "Unrecognizable coordinate file."
                    )
        ...
        if mode in ["spring", "line", "random"]:
            if isinstance(ChromSeq, list):
                ...
                if len(ChromSeq) > 1:
                    raise ValueError(
                        "'{}' mode can only be used to create single chains.".format(
                            mode
                        )
                    )
            ...
            if CoordFiles != None:
                raise ValueError(
                    "Providing coordinates' file not compatible with mode '{0}'.".format(
                        mode
                    )
                )
        ...
        if mode == "line":
            ...
            return self.createLine(
                ChromSeq=ChromSeq[0], chromosome=chromosome
            )

        elif mode == "spring":

            return self.createSpringSpiral(
                ChromSeq=ChromSeq[0],
                isRing=isRing,
                chromosome=chromosome,
            )
            ...
            if not ChromSeq is None:
                print(
                    "Attention! Sequence files are not considered for 'ndb' mode."
                )
    ...
    def saveStructure(self, filename=None, mode="gro"):
        ...
        if filename is None:
            ...
            if len(self.chains) > 1:
                ...
                filename_format = (
                    os.path.splitext(filename)[0] + ".{mode}"
                )
            ...
            for chainNum, chain in zip(
                ...
                ndbf.append(
                    header_string.format(
                        "HEADER",
                        "NDB File genereted by Open-MiChroM",
                        " ",
                        " ",
                    )
                )
                ndbf.append(
                    title_string.format(
                        "TITLE ",
                        "  ",
                        "A Scalable Computational Approach for ",
                    )
                )
                ndbf.append(
                    title_string.format(
                        "TITLE ",
                        "2 ",
                        "Simulating Complexes of Multiple Chromosomes",
                    )
                )
                ndbf.append(
                    expdata_string.format(
                        "EXPDTA",
                        "  ",
                        "Cell Line  @50k bp resolution",
                    )
                )
                ndbf.append(
                    expdata_string.format(
                        "EXPDTA", "  ", "Simulation - Open-MiChroM"
                    )
                )
                ndbf.append(
                    author_string.format(
                        "AUTHOR",
                        "  ",
                        "Antonio B. Oliveira Junior - 2020",
                    )
                )

                seqList = self.type_list_letter[
                    chain[0] : chain[1] + 1
                ]
                ...
                if len(self.diff_types) == len(self.data):
                    seqList = [
                        "SQ"
                        for x in range(len(self.type_list_letter))
                    ]
                ...
                for i, line in zip(
                    list(range(len(data_chain))), data_chain
                ):
                    ndbf.append(
                        ndb_string.format(
                            "CHROM",
    ...
    def initPositions(self):
        ...
        if not hasattr(self, "context"):
            raise ValueError(
                "No context; cannot set positions. Initialize the context before calling initPositions."
            )
    ...
    def initVelocities(self):
        ...
        if not hasattr(self, "context"):
            raise ValueError(
                "No context; cannot set velocities. Initialize the context before calling initVelocities."
            )
    ...
    def setFibPosition(self, positions, returnCM=False, factor=1.0):
        ...
        R_nucleus = (
            (self.chains[-1][1] + 1) * (1.0 / 2.0) ** 3 / 0.1
        ) ** (1.0 / 3.0)

        for i in range(len(self.chains)):
            points[i] = [x * factor * R_nucleus for x in points[i]]
            positions[
                self.chains[i][0] : self.chains[i][1] + 1
            ] += np.array(points[i])
        ...
        for n in self.forceDict:
            ...
            forceValues.append(
                self.context.getState(
                    getEnergy=True,
                    groups={self.forceDict[n].getForceGroup()},
                )
                .getPotentialEnergy()
